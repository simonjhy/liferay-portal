...
<p>
	<b><liferay-ui:message key="${className.toLowerCase()}.caption"/></b>
</p>
...
<aui:form action="<%= sendNotificationURL %>" method="post" name="fm2" >
	<aui:input name="<%= Constants.CMD %>" type="hidden" value="<%= ${className}PortletKeys.NOTIFY %>" />

	<aui:fieldset-group markupView="lexicon">
		<aui:fieldset>
			<aui:input type="text" name="<%= ${className}PortletKeys.USER_EMAIL %>" />
			<aui:button-row>
				<aui:button type="submit" value="<%= ${className}PortletKeys.NOTIFY %>" />
			</aui:button-row>
		</aui:fieldset>
	</aui:fieldset-group>
</aui:form>